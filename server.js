...
app.post('/api/deposit', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: `$${amount.toFixed(2)} deposited successfully.`, newBalance, transaction: newTransaction });
  } catch (error) {
...
app.post('/api/withdraw', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, message: `$${amount.toFixed(2)} withdrawn successfully.`, newBalance, transaction: newTransaction });
  } catch (error) {
...
app.post('/api/transfer', async (req, res) => {
  ...
  try {
    const senderResult = await pool.query(
      'SELECT id, name, balance, transactions, account_number FROM users WHERE email = $1',
      [email]
    );
    ...
    try {
      ...
      const senderFresh = await client.query(
        'SELECT balance, transactions FROM users WHERE email = $1 FOR UPDATE',
        [email]
      );
      let senderBalance = parseFloat(senderFresh.rows[0].balance);
      let senderTransactions = senderFresh.rows[0].transactions;
      if (typeof senderTransactions === 'string') senderTransactions = JSON.parse(senderTransactions);
      ...
      // ✅ DEDUCT balance immediately but mark as PENDING
      ...
      // ✅ Deduct from sender balance, save pending transaction
      await client.query(
        'UPDATE users SET balance = $1, transactions = $2::jsonb WHERE email = $3',
        [newSenderBalance, JSON.stringify(senderTransactions), email]
      );

      // ✅ Recipient does NOT receive funds yet (pending)
      // Recipient gets notified only when admin approves (future feature)
      ...
      res.json({
        success: true,
        message: `Transfer of $${amount.toFixed(2)} to ${recipient.name} is now pending. Amount has been deducted from your balance.`,
